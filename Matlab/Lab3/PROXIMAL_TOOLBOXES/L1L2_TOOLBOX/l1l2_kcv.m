function [cv_output,model, tau_values] = l1l2_kcv(X,Y,varargin)
...
%  if 'protocol'=='both': has both of the above sets of fields.
%
%   See also L1L2_ALGORITHM, L1L2_REGPATH, RLS_ALGORITHM
%
...
if isempty(tau_values);
    if isempty(tau_max);
        ...
        tau_max = l1l2_tau_max(Xtmp,Ytmp) / length(Y);
    end
    % set tau_min
    if isempty(tau_min);
        tau_min = tau_max/(100*length(Y));
    end
    % geometric series
...
for i = 1:length(sets);
    ...
    [selected{i},tmin,n_iter,beta] = l1l2_regpath(Xtr,Ytr,tau_values,'smooth_par',smooth_par);
